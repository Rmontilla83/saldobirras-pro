...
import { NO_ZONE_ID } from '@/data/stadium-seats';
...
export default function RegisterView({ onSubmit }: Props) {
  ...
  const handleSeatChange = (zone: string, row: string, seat: string) => {
    setSeatZone(zone);
    setSeatRow(row);
    setSeatNumber(seat);
  };
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit({
      ...
      seat_zone: seatZone && seatZone !== NO_ZONE_ID ? seatZone : undefined,
    ...
    setSeatZone(''); setSeatRow(''); setSeatNumber('');
    ...
    <div className="max-w-[440px] mx-auto animate-[fadeIn_0.25s_ease]">
      <div className="card">
        ...
        <div className="space-y-4">
          ...
          <SeatPicker zone={seatZone} row={seatRow} seat={seatNumber} onChange={handleSeatChange} />
